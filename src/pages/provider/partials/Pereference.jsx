...
const ServiceTypes = () => {
  ...
  useEffect(()=>{
    ...
    async function load(){
      setLoading(true)
      try{
        const data = await preferences.list()
        if(mounted){
  ...
  async function handleSave(){
    ...
    try{
      //تعديل
      if(editingId){
        const updated = await preferences.update(editingId, { name: formName })
        setServiceTypes(prev=> prev.map(s=> s.id===editingId ? {...s,...updated} : s))
        alert('تم التعديل بنجاح')
        // إضافة جديد
      } else {
        const created = await preferences.create({ name: formName })
  ...
  async function handleDelete(item){
    ...
    setLoading(true)
    try{
      await preferences.remove(item.id)
