...
export default function CategoryTreePage() {
  ...
  useEffect(() => {
    ...
    const loadprovider = async () => {
      setLoading(true);
      const { data, result, text } = await apiProvider.getProfile();
      console.log(data);
      if (result) {
        console.log(data);
  ...
  const handleSave = async() => {
    ...
    console.log("ids" ,ids);
    const {result  , text} = await apiProvider.updateProfile({ categories:ids});
    if (result)
  ...
  return (
    <>
      {loading && <MySpinner />}
      <div className="container-fluid pt-1 px-5 bg-white h-80vh">
        <div className="row">
          {/* القائمة الجانبية */}
